...
//コンストラクタ
HanshinTigers::HanshinTigers(int size){
...
//メンバへのアクセス関数
int HanshinTigers::Get(int i){
    Checkindex(i);
    return m_array[i];
}

int HanshinTigers::Set(int i, int value){
    Checkindex(i);
    m_array[i] = value;
}

//インデックスのチェック
void HanshinTigers::Checkindex(int i){
    if(0 <= i && i < m_size){
        //インデックスは正常です
    }else{
        cout << "インデックスは不正です" << endl << "値" << i <<endl;
